...
import org.apache.spark.lineage.{Direction, HashAwarePartitioner, LineageHashing, LocalityAwarePartitioner}
...
//    prevResult.map(r => (r._1, r._2._1))
  ...
  override def saveAsCSVFile(path: String): Unit = {
    def merge(srcPath: String, dstPath: String): Unit =  {
      ...
      // Hadoop 3 removed FileUtil.copyMerge; inline equivalent (deleteSource = false).
      val out = hdfs.create(new Path(dstPath))
      try {
        hdfs.listStatus(new Path(srcPath)).filter(_.isFile).sortBy(_.getPath.getName)
          .foreach { status =>
            val in = hdfs.open(status.getPath)
            try {
              org.apache.hadoop.io.IOUtils.copyBytes(in, out, hadoopConfig, false)
            } finally {
              in.close()
            }
          }
          ...
          case _: TapPostCoGroupLRDD[_] =>
            ...
            val right =
              position.get.getCachedData.setCaptureLineage(true).map {
                ...
                ).flatMap( r => for(v <- r._2) yield((v._2, LineageHashing.hashKey(r._1)), ((r._1, v._1), v._2).toString()))
          ...
          case _: TapPreCoGroupLRDD[_] =>
            ...
            val right = new ShuffledLRDD[RecordId, Any, Any](position.get.getCachedData.setCaptureLineage(true)
              ...
              case (v, Array(vs, w1s)) =>
                ...
                r => for(v <- r.productIterator) yield v.asInstanceOf[(_, Iterable[(_, Int)])]
              ).flatMap( r => for(v <- r._2) yield((v._2, LineageHashing.hashKey(r._1)), ((r._1, v._1), v._2).toString())), part).map(r => ((r._1._1, r._1._2), r._2))
...
//            result = result.cache()
          case post : TapPostShuffleLRDD[(Any, Long) @unchecked] =>
            ...
            result = new ShowRDD(rightJoin[Int, Any](current,post.getCachedData.map(r => (LineageHashing.hashKey(r._1), r)))
