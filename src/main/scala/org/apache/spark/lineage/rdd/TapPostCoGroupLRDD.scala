...
import org.apache.spark.lineage.{LineageContext, LineageHashing}
import org.apache.spark.lineage.util.LongLongByteBuffer
import org.apache.spark.util.PackIntIntoLong
import org.apache.spark.util.collection.{CompactBuffer, OpenHashMap, OpenHashSet}
  ...
  ) extends TapPostShuffleLRDD[T](lc, deps)
{
  ...
  override def materializeBuffer: Array[Any] = {
    if(buffer != null) {
      val map: OpenHashMap[Int, CompactBuffer[Long]] = new OpenHashMap()
      ...
      if(isLast) {
        set.iterator
          ...
          .map(r => (PackIntIntoLong(splitId, r._2), ((if (map.contains(r._1)) map(r._1) else null), r._1))
          ).toArray
      } else {
        set.iterator
          .map(r => (PackIntIntoLong.getLeft(r), PackIntIntoLong.getRight(r)))
          .map(r => (r._2.toLong, ((if (map.contains(r._1)) map(r._1) else null), r._1))
          ).toArray
      }
    } else {
      Array()
  ...
  override def tap(record: T) = {
    ...
    val hash = LineageHashing.hashKey(key)
    ...
    // Fork reversed the array here to compensate for CoGroupedLRDD.tapLeft wrapping
    // parents in reverse (Stack) order; taps are now spliced in dependency order, so
    // the slots are already correct.
