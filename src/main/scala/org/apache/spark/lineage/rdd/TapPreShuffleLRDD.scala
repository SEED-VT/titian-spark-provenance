...
import org.apache.spark.lineage.{Int2RoaringBitMapOpenHashMap, LineageContext, LineageHashing}
...
class TapPreShuffleLRDD[T <: Product2[_, _]: ClassTag](
  ...
  override def tap(record: T) = {
    buffer.put(LineageHashing.hashKey(record._1), tContext.currentInputId)
    // Fork: the modified ExternalSorter attached this packed id to each value during the
    // shuffle write (insertAll "grouping" variant / writePartitionedFile). Emitting the
    // tagged record from the tap itself produces the identical wire format with the
    // stock writer; LAggregator's wrapped map-side functions keep the tag through any
    // map-side combine (collapsing it to (partition, 0), as the fork did).
    (record._1, (record._2, PackIntIntoLong(splitId, tContext.currentInputId))).asInstanceOf[T]
  }
}
