...
import org.apache.spark.lineage.{LineageContext, LineageManager, LineageTaskState}
...
class TapLRDD[T: ClassTag](@transient lc: LineageContext, @transient deps: Seq[Dependency[_]])
  ...
  @transient private[spark] var tContext: LineageTaskState = _
  ...
  override def compute(split: Partition, context: TaskContext) = {
    if(tContext == null) {
      tContext = LineageTaskState.get(context)
    }
