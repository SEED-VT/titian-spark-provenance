...
trait Lineage[T] extends RDD[T] {
  ...
  // The fork added the four members below to Spark's RDD itself; as a library they live
  // on the Lineage trait — only lineage-aware RDDs ever carried meaningful values.
  ...
  def materialize = {
    // Fork: assigned RDD.storageLevel directly (it had been made protected). Public
    // persist matches: same level on re-call is a no-op, never an override conflict.
