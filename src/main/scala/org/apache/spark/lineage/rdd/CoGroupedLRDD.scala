...
class CoGroupedLRDD[K: ClassTag](var lrdds: Seq[RDD[_ <: Product2[K, _]]], part: Partitioner)
  ...
  // A queue (the fork used a Stack) so that successive tapLeft calls wrap the parents
  // in dependency order — tap i must feed shuffle dependency i, or the cogroup value
  // slots come out crossed.
