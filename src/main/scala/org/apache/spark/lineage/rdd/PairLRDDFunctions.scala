...
import org.apache.spark.lineage.{LAggregator, LineageHashPartitioner}
...
(implicit kt: ClassTag[K], vt: ClassTag[V], ord: Ordering[K] = null)
  ...
  override def combineByKeyWithClassTag[C](
    ...
    // Capture-time shuffles must place keys by the stored murmur3 hash so trace-time
    // joins can locate them (fork: global HashPartitioner replacement; see
    // LineageHashPartitioner).
    val lineagePartitioner =
      if (lineageContext.isLineageActive && partitioner.isInstanceOf[HashPartitioner]) {
        new LineageHashPartitioner(partitioner.numPartitions)
      } else {
        partitioner
      }

      new ShuffledLRDD[K, V, C](self, lineagePartitioner)
  ...
  override def saveAsHadoopFile(
    ...
    FileOutputFormat.setOutputPath(hadoopConf,
      org.apache.spark.internal.io.SparkHadoopWriterUtils.createPathFromString(path, hadoopConf))
  ...
  override def saveAsHadoopDataset(conf: JobConf): Unit = {
    // Fork: re-implemented the 2.1-era SparkHadoopWriter loop so the save ran through
    // LineageContext.runJob (which taps the DAG). The only lineage-relevant part is the
    // tapping; tap first, then delegate to the stock (Spark 4) writer.
    val rddToSave: RDD[(K, V)] = if (self.lineageContext.isLineageActive) {
      self.lineageContext.tapJob(self).asInstanceOf[RDD[(K, V)]]
    } else {
      self
    }
    new PairRDDFunctions(rddToSave).saveAsHadoopDataset(conf)
  }
}
